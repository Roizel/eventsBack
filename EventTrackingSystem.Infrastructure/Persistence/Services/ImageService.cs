    ...
    ) : IImageService
{
    ...
    {
        var videoTypes = new[] { ".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flv", ".webm" };
        var extension = Path.GetExtension(file.FileName).ToLowerInvariant();
    ...
    public bool IsVideoMimeType(IFormFile file)
    {
        ...
        return videoMimeTypes.Contains(file.ContentType.ToLowerInvariant());
    }
}